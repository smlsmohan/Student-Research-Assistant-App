...
ALTER TABLE IF EXISTS user_search_counts ENABLE ROW LEVEL SECURITY;
...
-- Create user_search_history table to store individual searches
...
-- Create index for search counts
CREATE UNIQUE INDEX IF NOT EXISTS user_search_counts_user_id_idx 
ON user_search_counts(user_id);
...
-- RLS Policies for user_search_counts
DROP POLICY IF EXISTS "Users can view own search count" ON user_search_counts;
CREATE POLICY "Users can view own search count" ON user_search_counts
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own search count" ON user_search_counts;
CREATE POLICY "Users can insert own search count" ON user_search_counts
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own search count" ON user_search_counts;
CREATE POLICY "Users can update own search count" ON user_search_counts
...
-- Trigger for user_search_counts updated_at
DROP TRIGGER IF EXISTS update_user_search_counts_updated_at ON user_search_counts;
...
BEGIN
  ...
  -- Initialize search count for new user
  INSERT INTO public.user_search_counts (user_id, search_count)
  VALUES (NEW.id, 0);
